...
def exec_wrapper_cmd(exec_args, exec_with, cmd_list):
    wrapper = []
    target = ''
    ...
    wrapper += exec_args + ['--', target + cmd_list[0]] + cmd_list[1:]
    return wrapper

def exec_with_wrapper(exec_args, exec_with, install_dir, cmd_list):
    ...
    if exec_with is not None:
        cmd_list = exec_wrapper_cmd(exec_args, exec_with, cmd_list)
