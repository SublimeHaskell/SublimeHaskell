...
Please consider compiling `hsdev` with GHC 8.0.1, **not GHC 8.0.2**. It isn't clear whether the GHC 8.0.2 runtime changed in an inexplicable way or if `hsdev` itself is not flushing its network buffers when sending a response. Either way, `hsdev` compiled with GHC 8.0.2 is not reliable and will lock up the SublimeText 3 main UI thread.
